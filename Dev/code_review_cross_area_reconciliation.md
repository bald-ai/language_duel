...
Area 6 #3 wants self-duel modeled as single-actor PvE instead of a mirrored PvP row. Area 7 confirms the *client* side is already clean (`isSelfDuelSelection.ts`, mode-picker hidden for self-duel). **Resolution:** keep Area 7's client helper; if Area 6 reshapes the self-duel write model, re-check the lobby's `createSelfDuel` call path aligns. Dependency, not a conflict.
...
5. **Hint naming drift (Area 4 #8 ↔ Area 6).** `+15 Seconds` label vs `plus_ten_seconds`/`HINT_PLUS_TEN_BONUS_SECONDS` — canonical `HintType`/constants live in `lib/hintPool/*` (Area 6); rename there, not just at `HintPoolUI`.
