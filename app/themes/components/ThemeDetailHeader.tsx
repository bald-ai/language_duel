            ...
            {canEdit && (
              <div className="flex flex-wrap gap-2 justify-start sm:justify-end">
                <button
                  ...
                  data-testid={`theme-add-${contentType}`}
                >
...
function ThemeNameEditor({
  themeName,
  canEdit,
  onThemeNameChange,
}: Pick<
  ThemeDetailHeaderProps,
  "themeName" | "canEdit" | "onThemeNameChange"
>) {
  const colors = useAppearanceColors();
...
function ThemeVisibilityControls({
  isOwner,
  visibility,
  isUpdatingVisibility,
  onVisibilityChange,
  ...
  ThemeDetailHeaderProps,
  "isOwner" | "visibility" | "isUpdatingVisibility" | "onVisibilityChange"
>) {
  const colors = useAppearanceColors();
  ...
  if (!isOwner || !onVisibilityChange) return null;
  return (
    <div
...
function ThemeFriendEditingControl({
  isOwner,
  visibility,
  friendsCanEdit,
  isUpdatingFriendsCanEdit,
...
>) {
  ...
  if (!isOwner || visibility !== "shared" || !onFriendsCanEditChange)
    return null;
  return (
    <button
      ...
      title={
        friendsCanEdit
          ? "Friends can edit - Click to lock"
          : "Friends can view only - Click to unlock"
      }
...
function ThemeTtsControls({
  ...
  ThemeDetailHeaderProps,
  "canEdit" | "isGeneratingTTS" | "isTTSUpToDate" | "onGenerateTTS"
>) {
  const colors = useAppearanceColors();
  if (!canEdit || !onGenerateTTS) return null;
  return (
    <>
      <button
