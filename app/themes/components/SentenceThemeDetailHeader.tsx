...
interface SentenceThemeDetailHeaderProps {
  themeName: string;
  isOwner: boolean;
  canEdit: boolean;
...
export function SentenceThemeDetailHeader({
  ...
  const handleThemeNameKeyDown = (event: React.KeyboardEvent) => {
    if (event.key === "Enter") commitThemeName();
    else if (event.key === "Escape") setIsEditingThemeName(false);
  };
                ...
                <input
                  ...
                  onChange={(event) => {
                    if (event.target.value.length <= THEME_NAME_MAX_LENGTH) {
                      setEditedThemeName(event.target.value);
                    }
                  }}
            ...
            {canEdit && (
              <div className="flex flex-wrap gap-2 justify-start sm:justify-end">
                <button
                  ...
                  data-testid="theme-add-sentence"
                >
                  + Add Sentence
                </button>
                <button
