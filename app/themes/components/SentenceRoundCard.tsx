...
>) {
  const colors = useAppearanceColors();
  const hasIssue = hasSentenceRowIssues(issues);
      ...
      <div className="ml-auto flex items-center gap-2">
        <button
          ...
          title={
            hasGeneratedTts
              ? "Play generated TTS"
              : "Generate TTS for this sentence first"
          }
...
function SentenceEnglishField({
  ...
  SentenceRoundCardProps,
  "round" | "index" | "issues" | "canEdit" | "onEditField"
>) {
  const colors = useAppearanceColors();
...
function SentenceSpanishStage({
  ...
  SentenceRoundCardProps,
  "round" | "index" | "issues" | "canEdit" | "onEditField" | "onToggleFreeWord"
>) {
  const colors = useAppearanceColors();
      ...
      {spanishTokens.length > 0 ? (
        <div className="flex flex-wrap justify-center items-end gap-x-3.5 gap-y-2 mt-4 pb-1">
          {spanishTokens.map((token, tokenIndex) => {
            ...
            const meaning =
              wordMeanings[tokenIndex] ?? SENTENCE_WORD_MEANING_PLACEHOLDER;
            return (
              <button
    ...
    <div className="grid grid-cols-3 gap-2">
      {distractorSlots.map((distractor, distractorIndex) => (
        <button
          key={distractorIndex}
          onClick={() =>
            canEdit && onEditField(index, "distractor", distractorIndex)
          }
          disabled={!canEdit}
...
function hasSentenceRowIssues(issues: SentenceRowIssues): boolean {
  return (
