...
export const ThemeWordCard = memo(function ThemeWordCard({
  ...
  const issueMessage = issues.repairIssue?.cardMessage ?? null;
  ...
  return (
    ...
    >
      <ThemeWordHeader word={word} index={index} issues={issues} canEdit={canEdit}
        playingWordKey={playingWordKey} onDeleteWord={onDeleteWord} onPlayWordTTS={onPlayWordTTS} />
      <ThemeWordPrompt word={word} index={index} issues={issues} canEdit={canEdit} onEditWord={onEditWord} />
      <ThemeWordAnswer word={word} index={index} issues={issues} canEdit={canEdit} onEditWord={onEditWord} />
      <ThemeWordDistractors word={word} index={index} issues={issues} canEdit={canEdit} onEditWord={onEditWord} />
...
type ThemeWordFieldProps = Pick<ThemeWordCardProps, "word" | "index" | "issues" | "canEdit" | "onEditWord">;

function ThemeWordHeader({word, index, issues, canEdit, playingWordKey, onDeleteWord, onPlayWordTTS}:
  Omit<ThemeWordCardProps, "onEditWord">) {
  const colors = useAppearanceColors();
  const { repairIssue } = issues;
...
function ThemeWordPrompt({word, index, issues, canEdit, onEditWord}: ThemeWordFieldProps) {
  const colors = useAppearanceColors();
  const { isDuplicate } = issues;
...
function ThemeWordAnswer({word, index, issues, canEdit, onEditWord}: ThemeWordFieldProps) {
  const colors = useAppearanceColors();
  const wrongMatchesAnswer = issues.wrongMatchesAnswerIndices.size > 0;
...
function ThemeWordDistractors({word, index, issues, canEdit, onEditWord}: ThemeWordFieldProps) {
  const colors = useAppearanceColors();
  const { duplicateWrongIndices, wrongMatchesAnswerIndices } = issues;
