...
export function ThemeList({
  ...
  return (
    <>
      <ThemeListHeader
        count={tabbedThemes.length}
        filterDisplay={filterDisplay}
        isFiltering={isFiltering}
        showArchived={showArchived}
        onOpenFriendFilter={onOpenFriendFilter}
        onClearFriendFilter={onClearFriendFilter}
        onToggleShowArchived={onToggleShowArchived}
        onGenerateNew={onGenerateNew}
        contentTypeTab={contentTypeTab}
        onContentTypeTabChange={onContentTypeTabChange}
      />
...
> & {
  count: number;
  filterDisplay: string | null;
  isFiltering: boolean;
  showArchived: boolean;
  contentTypeTab: ContentTypeTab;
}) {
  const colors = useAppearanceColors();
  const countLabel = `${count} theme${count !== 1 ? "s" : ""}`;
  const subtitle = filterDisplay
    ? `Filtering: ${filterDisplay} • ${countLabel}`
