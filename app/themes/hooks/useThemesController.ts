...
export function useThemesController() {
  ...
  const goBack = useCallback(() => {
    ...
    if (viewMode === VIEW_MODES.EDIT_WORD) {
      ...
      navigateBackFromThemeList(router);
    }
  }, [
    detail,
    generation,
  ...
  const detailProps = buildWordThemeDetailProps(
    detail,
    generation,
    wordEdit,
    themeActions,
  ...
  const sentenceDetailProps = buildSentenceThemeDetailProps(sentenceController);

  const sentenceEditorProps = buildSentenceEditorProps(sentenceController);
  ...
  return {
    ...
    deleteConfirmProps: buildDeleteConfirmProps(
      deleteConfirm,
      detail,
      confirmDeleteTheme,
      () => setDeleteConfirm(null),
...
function buildWordThemeDetailProps(
  detail: ReturnType<typeof useThemeDetailController>,
  generation: ReturnType<typeof useThemeGenerationController>,
  wordEdit: ReturnType<typeof useThemeWordEditController>,
  themeActions: ReturnType<typeof useThemeActions>,
  tts: ReturnType<typeof useThemeTtsController>,
) {
  return {
...
function buildSentenceThemeDetailProps(
  sentenceController: ReturnType<typeof useSentenceThemeController>,
) {
  return sentenceController.selectedTheme
    ? {
...
function buildSentenceEditorProps(
  sentenceController: ReturnType<typeof useSentenceThemeController>,
) {
  return sentenceController.editField
    ? {
        themeName: sentenceController.selectedTheme?.name ?? "",
...
function navigateBackFromThemeList(router: ReturnType<typeof useRouter>) {
...
function buildDeleteConfirmProps(
  deleteConfirm: DeleteConfirmState | null,
  detail: ReturnType<typeof useThemeDetailController>,
  confirmDeleteTheme: () => Promise<void>,
  onCancel: () => void,
) {
  const selection: DeleteConfirmState = deleteConfirm ?? { type: "word" };
  return {
    isOpen: deleteConfirm !== null,
    itemName: selection.themeName || selection.wordName || "",
