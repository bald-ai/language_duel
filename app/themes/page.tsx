...
const GenerateThemeModal = dynamic(
  () =>
    import("./components/GenerateThemeModal").then(
      (mod) => mod.GenerateThemeModal,
    ),
  { loading: () => null },
);
const DeleteConfirmModal = dynamic(
  () =>
    import("./components/DeleteConfirmModal").then(
...
const GenerateMoreModal = dynamic(
  () =>
    import("./components/GenerateMoreModal").then(
      (mod) => mod.GenerateMoreModal,
    ),
  { loading: () => null },
);
const DiscardPickAndPruneModal = dynamic(
  () =>
    import("./components/DiscardPickAndPruneModal").then(
      (mod) => mod.DiscardPickAndPruneModal,
    ),
  { loading: () => null },
);
const PickAndPruneReview = dynamic(
  () =>
    import("./components/PickAndPruneReview").then(
      (mod) => mod.PickAndPruneReview,
    ),
  { loading: () => null },
);
...
const FriendFilterModal = dynamic(
  () =>
    import("./components/FriendFilterModal").then(
      (mod) => mod.FriendFilterModal,
    ),
  { loading: () => null },
);
const SentenceThemeDetail = dynamic(
  () =>
    import("./components/SentenceThemeDetail").then(
      (mod) => mod.SentenceThemeDetail,
    ),
  { loading: () => null },
);
const SentenceRoundEditor = dynamic(
  () =>
    import("./components/SentenceRoundEditor").then(
      (mod) => mod.SentenceRoundEditor,
    ),
  { loading: () => null },
);
...
const ThemeContentTypeModal = dynamic(
  () =>
    import("./components/ThemeContentTypeModal").then(
      (mod) => mod.ThemeContentTypeModal,
    ),
  { loading: () => null },
);
  ...
  return (
    <ThemedPage className="h-dvh overflow-hidden">
      <div className="relative z-10 flex-1 min-h-0 flex flex-col items-center justify-start w-full max-w-xl mx-auto px-6 pt-6 pb-4">
        <ThemeListContent controller={controller} />

        {controller.isSentenceReviewActive && (
          <PickAndPruneSentenceReview
            {...controller.sentencePickAndPruneReviewProps}
          />
        )}

        <WordThemeContent controller={controller} />

        <SentenceThemeContent controller={controller} />
      </div>
      ...
      <GenerateMoreSentenceRoundsModal
        {...controller.sentenceGenerateMoreModalProps}
      />
...
type ContentProps = { controller: ReturnType<typeof useThemesController> };
function ThemeListContent({ controller }: ContentProps) {
  return (
    <>
...
function WordThemeContent({ controller }: ContentProps) {
  return (
    <>
      {controller.viewMode === VIEW_MODES.DETAIL &&
        !controller.isSentenceFlowActive &&
        controller.selectedTheme && <ThemeDetail {...controller.detailProps} />}
...
function SentenceThemeContent({ controller }: ContentProps) {
  // A field editor replaces the detail surface; review has priority over both.
  if (controller.isSentenceReviewActive) return null;
  if (controller.sentenceEditField !== null) {
    return controller.sentenceEditorProps ? (
      <SentenceRoundEditor {...controller.sentenceEditorProps} />
    ) : null;
  }
  if (controller.isSentenceFlowActive && controller.sentenceDetailProps) {
    return <SentenceThemeDetail {...controller.sentenceDetailProps} />;
  }
  return null;
}
