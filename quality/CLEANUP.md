...
- Ordinary tests: `npm run test:run`, plus the lint and TypeScript checks from `AGENTS.md`
...
- Mutation tests: `python3 scripts/verification-session.py mutations <name>`. Cases are defined in `quality/mutations.py`.
- Critical rules, which should get close to 100% coverage and mutation tests: scoring, charging and refunding credits, duel and relay state, sabotage timing, weekly goal status, theme access, and generation request validation.
