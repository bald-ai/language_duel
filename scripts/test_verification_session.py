...
class VerificationSessionTests(unittest.TestCase):
    def test_coverage_does_not_remove_unrelated_build_files(self):
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            unrelated = root / '.build' / 'example' / 'codecov' / 'keep'
            unrelated.parent.mkdir(parents=True)
            unrelated.write_text('existing')
            self.assertEqual(runner.run_session([], [], root=root, coverage=True), 0)
            self.assertEqual(unrelated.read_text(), 'existing')
