...
Ordinary tests: `npm run test:run`. Existing lint and TypeScript checks remain in the handoff instructions. `test:coverage`, `quality:check`, `quality:metrics`, and `quality:mutations` are aliases for the manual runner. Coverage-only retains the configured 70% thresholds. Metrics collect fresh Istanbul counters and use the existing component gates. Mutation names can follow `mutations`.
