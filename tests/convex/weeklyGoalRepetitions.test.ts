...
describe("weekly goal spaced repetition", () => {
  ...
  it("createRepetitionChallenge rejects Tag Team mode", async () => {
    vi.spyOn(Date, "now").mockReturnValue(READY_NOW);
    const db = new InMemoryDb();
    seedCompletedGoal(db);
    ...
    const handler = (createRepetitionChallenge as unknown as {
      _handler: (
        ctx: unknown,
        args: { weeklyGoalId: Id<"weeklyGoals">; duelMode: "pvp" | "pve" | "relay" | "tbt" }
      ) => Promise<Id<"challenges">>;
    })._handler;
    ...
    ).rejects.toThrow("Tag Team is not available for spaced repetition duels");
  });
