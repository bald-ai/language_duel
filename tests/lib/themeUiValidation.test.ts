...
describe("theme UI validation", () => {
  ...
  it("does not expose server-only length issues as UI repair issues", () => {
    const words: WordEntry[] = [
      { word: "", answer: "el gato", wrongAnswers: ["a", "b", "c"] },
    ];

    expect(getThemeRepairIssueForWords(words)).toBe(null);
    expect(getThemeSaveErrorMessage(words)).toBe(null);
  });
});
