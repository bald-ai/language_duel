...
/**
 * ADMIN: One-off cleanup before the duel item-index schema rename.
 *
 * Run before deploying the schema that renames stored duel fields. It clears
 * only transient duel gameplay data and leaves weeklyGoalRepetitions intact,
 * because repetition rows are retained progress history.
 */
export const clearTransientDuelDataForItemIndexRename = internalMutation({
  args: {
    confirm: v.literal("clear_transient_duel_data"),
  },
  handler: async (ctx) => {
    const deletionReport = {
      duels: 0,
    ...
    const emailLogs = await ctx.db.query("emailNotificationLog").collect();
    for (const emailLog of emailLogs) {
      if (!emailLog.challengeId && !emailLog.duelId) continue;
      await ctx.db.delete(emailLog._id);
      deletionReport.emailNotificationLog++;
    }

    return {
      success: true,
      deletionReport,
      message: "Cleared transient duel data for item-index schema rename.",
    };
  },
});
