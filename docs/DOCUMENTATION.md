...
- Solo practice: A user practices against the app without needing another player. The ad-hoc Learn + Practice flow and weekly-goal solo practice both support word themes, sentence themes, or mixed selections. Sentence practice uses a fill-the-blanks ladder where higher levels blank more of the sentence until the user builds the whole sentence. Boss solo practice and spaced repetition remain word-only.
...
- The PvE WORD hint pool has four one-use hint types: `50/50`, `+15 Seconds`, `Anagram`, and `Letter Count`.
...
- Sentence rounds now share ONE structure across every mode (PvP / PvE / self-duel): build-and-confirm, built strictly in order — tap tiles into slots, peel the last, verify the whole sentence on Confirm. There is no per-tap validation (the old per-tap PvE board is retired); a wrong tile can occupy a slot and the whole sentence is graded on Confirm. Scoring is the build-and-confirm ladder for every mode (0 failed Confirms → +1, 1 → 0, ≥2 → −1, with a −1 floor); a boss/SR sentence round loses one life per unsolved round (on Confirm), not per wrong tap. Every sentence mode uses one timer, `SENTENCE_TIMER_SECONDS = 60`. Only the footer tools differ by mode, exactly like word rounds.
- Sentence rounds get their OWN PvE hint pool (separate budget from the word pool, `sentenceHintPoolUsed`): three one-use hints — `Freeze (+30s)`, `Remove distractors` (greys out every decoy tile), and `Reveal 2 tiles` (marks up to 2 correct slots, never the whole sentence). Same shared-team semantics as the word pool (either player fires, hits both boards, no consent, one hint per question). Every sentence hint also grants a universal +10s; Freeze is the dedicated time hint at 30s total. The reveal mark is a top-right amber slot badge that turns green ✓ once the tile is placed in its slot, with a gentle pulse on the next-due tile; marks persist through Reset. The accumulated hint time rides on `currentQuestionTimerBonusSeconds` (the single timer source — the mutation never pushes `questionStartTime`).
